...
function Homepage() {
  ...
  const { user } = useSelector((state) => state.auth);
  const [problems, setProblems] = useState([]);
  const [searchTerm, setSearchTerm] = useState(""); // This state is now used
  const [solvedProblems, setSolvedProblems] = useState([]);
  const [filters, setFilters] = useState({
    difficulty: "all",
    tag: "all",
  ...
  // --- 2. Reset to page 1 whenever filters or search term change ---
  useEffect(() => {
    setCurrentPage(1);
  }, [filters, searchTerm]); // Added searchTerm to dependency array


  // --- UPDATED: Filtering logic now includes the search term ---
  const filteredProblems = problems.filter((problem) => {
          ...
          <div className="mb-8">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full bg-gray-800 border border-gray-700 text-white rounded-xl py-3 pl-12 pr-4 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500 appearance-none"
                value={searchTerm}
